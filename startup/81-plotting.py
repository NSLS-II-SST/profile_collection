...
def quick_view(hdr):#don't use this anymore, sw_viewer is better, more general
    ...
    if not fig.axes:
        ...
        zpw = ZoomPan()
        scale = 1.5
        figZooms = zps.zoom_factory(saxs_ax, base_scale=scale,joint=False)
        figZoomw = zpw.zoom_factory(waxs_ax, base_scale=scale,joint=False)
        figPans = zps.pan_factory(saxs_ax,joint=False)
        figPanw = zpw.pan_factory(waxs_ax,joint=False)
    else:
