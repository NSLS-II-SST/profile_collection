...
def full_oxygen_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    sample()
    md['enscan_type'] = 'full_oxygen_scan_nd'
    #beamline_status()
    ...
    yield from en_scan_core(sigs, dets, energy, energies, shuttervalue, times,md=md)
...
def short_oxygen_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    sample()
    md['enscan_type'] = 'short_oxygen_scan_nd'
    #beamline_status()
    if len(read_input("Starting a Oxygen energy scan hit enter in the next 3 seconds to abort", "abort", "", 3)) > 0:
        return
    mir3.Pitch.put(7.89)
...
def short_fluorine_scan_nd2(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    sample()
    md['enscan_type'] = 'short_fluorine_scan_nd2'
    #beamline_status()
    if len(read_input("Starting a Oxygen energy scan hit enter in the next 3 seconds to abort", "abort", "", 3)) > 0:
        return
    mir3.Pitch.put(7.89)
...
def short_fluorine_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    sample()
    md['enscan_type'] = 'short_fluorine_scan_nd'
    #beamline_status()
...
def full_nitrogen_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    md['enscan_type'] = 'full_nitrogen_scan_nd'
    sample()
    #beamline_status()
...
def very_short_carbon_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    md['enscan_type'] = 'very_short_carbon_scan_nd'
    sample()
    #beamline_status()
...
def short_carbon_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    sample()
    md['enscan_type'] = 'short_carbon_scan_nd'
...
def full_carbon_scan_nd(multiple=1,sigs=[Beamstop_SAXS,
    ...
    '''
    md['enscan_type'] = 'full_carbon_scan_nd'
    sample()
    ...
    # print(dets)
    # use these energies and exposure times to scan energy and record detectors and signals
    yield from en_scan_core(sigs, dets, energy, energies, shutter_values, times,md=md)


# @dark_frames_enable
def en_scan_core(I400sigs, dets, energy, energies, shuttervalues, times,md=None):
    ...
    yield from bp.scan_nd(I400sigs+[en.energy]+ dets,sigcycler,md=md)
...
def carbon_NEXAFS(exp_time=.2, gain_bs=6,s_or_w='w'):
    ...
    yield from bp.scan([IzeroMesh,switch[s_or_w]],en,270,340,351,md=md)



def oxygen_NEXAFS(exp_time=.2, gain_bs=6,s_or_w='w'):

    md['enscan_type'] = 'oxygen_NEXAFS'
    #Oct 2019, this pitch value seems to be optimal for carbon
    mir3.Pitch.put(7.89)
    IzeroMesh.set_exposure(exp_time)
    Beamstop_WAXS.set_exposure(exp_time)
    ...
    switch = {'s' : Beamstop_SAXS , 'w' : Beamstop_WAXS}
    RE.md['project_name'] = 'NEXAFS'
    yield from bp.scan([IzeroMesh,switch[s_or_w]],en,510,560,201,md=md)
...
def fluorine_NEXAFS(exp_time=.2, gain_bs=6,s_or_w='w'):
    ...
    md['enscan_type'] = 'fluorine_NEXAFS'
    #Oct 2019, this pitch value seems to be optimal for carbon
    mir3.Pitch.put(7.89)
    IzeroMesh.set_exposure(exp_time)
    Beamstop_WAXS.set_exposure(exp_time)
    ...
    switch = {'s' : Beamstop_SAXS , 'w' : Beamstop_WAXS}
    RE.md['project_name'] = 'NEXAFS'
    yield from bp.scan([IzeroMesh,switch[s_or_w]],en,670,720,201,md=md)


def fluorine_SAXS(exp_time=2):

    md['enscan_type'] = 'fluorine_SAXS'
    #Oct 2019, this pitch value seems to be optimal for carbon
    mir3.Pitch.put(7.89)
    set_exposure(exp_time)
    yield from bp.scan([sw_det, en.energy, Beamstop_SAXS, IzeroMesh],en,680,700,41,md=md)

def fluorine_WAXS(exp_time=2):

    md['enscan_type'] = 'fluorine_WAXS'
    #Oct 2019, this pitch value seems to be optimal for carbon
    mir3.Pitch.put(7.89)
    set_exposure(exp_time)
    yield from bp.scan([sw_det, en.energy, Beamstop_WAXS, IzeroMesh],en,680,700,41,md=md)



def calcium_NEXAFS(exp_time=2, gain_bs=5,s_or_w='w'):

    md['enscan_type'] = 'calcium_NEXAFS'
    IzeroMesh.set_exposure(exp_time)
    Beamstop_WAXS.set_exposure(exp_time)
    RSoXS_Diodes.gain_save = gain_bs
    switch = {'s' : Beamstop_SAXS , 'w' : Beamstop_WAXS}
    RE.md['project_name'] = 'NEXAFS'
    yield from bp.scan([IzeroMesh,switch[s_or_w]],en,320,360,201,md=md)
