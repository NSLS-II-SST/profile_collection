...
def alphaOnBlazeA(eV, k_invmm, m, blaze_deg):
    ...
    hc = 0.0012398  # in units of eV mm
    lambda_mm = hc / eV
    alpha = np.radians(blaze_deg)+np.arccos((m*k_invmm*lambda_mm)/(2*np.sin(np.radians(blaze_deg))))
    return (np.degrees(alpha))
...
def onBlazeFn(alpha_deg, *args):
    ...
    lambda_mm = hc / eV
    try:
        result = np.degrees(bl-0.5*(alpha+np.arcsin(m*k_invmm*lambda_mm-np.sin(alpha))))
    except:
        result = float('nan')
    return result
...
def ruben2005eqn8m(eV, c, k, m):
    ...
    return np.degrees(np.arcsin(-m*k*llambda*aa+np.sqrt(1+(c*m*k*llambda*aa)**2)))
...
def softXrayMono1(eV, k, m, c, rb_mm, bounce, inOff_deg, outOff_deg, verbose):
    ...
    # include offsets
    thetaPMinc_deg = abs(+0.5*(outOff_deg-inOff_deg+a*(180.-alpha_deg+beta_deg)))
    thetaPM_deg = +0.5*(outOff_deg+inOff_deg+a*(180.-alpha_deg+beta_deg))
    thetaGR_deg = a*(90.+beta_deg)+outOff_deg
    disp = getLinDisp(alpha_deg, beta_deg, k, m, rb_mm)
    if verbose:
        # alpha, beta both relative to normal and surface
        print('eV=', eV, 'c=', c)
        print('alpha=', alpha_deg, 90.-alpha_deg)
